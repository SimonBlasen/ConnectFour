...
public class BadPlayer implements SogoPlayer {
	...
	private int advantage10 = 0;

	private SogoMove takeMove;
	private SogoMove lastTookMovetakeMove;
	...
	public void generateNextMove(SogoGameConsole c) {
		...
		boolean advSetInThisSet = false;
		
		takeMove = moves.get(0);
		lastTookMovetakeMove = moves.get(0);
		...
		for (int depth = 4; depth < 30; depth++)
		{
			...
			{
				if (takeMove.i != lastTookMovetakeMove.i && takeMove.j != lastTookMovetakeMove.j && depth >= 10 && advSetInThisSet == false)
				{
					advSetInThisSet = true;
					advantage10++;
				}
				...
				lastTookMovetakeMove = new SogoMove(takeMove.i, takeMove.j);
		...
		System.out.println("Advantages 10 are: " + advantage10);
	}
	...
	public static double evaluateGame(long bp1, long bp2, boolean selfTurn)
	{
		...
		if (selfTurn == false)
		{
			boolean hasIt = hasCloseableThreeLine(all3LinesP2, bp2, bp1);
			if (hasIt == false)
	...
	public double evaluateOddEven(long bp1, long bp2, long oldBp1)
	{
		...
		return (difference * 100.0) + evaluateGame(bp1, bp2, oldBp1 == bp1) * 0.06;
	}
